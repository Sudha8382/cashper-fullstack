...
from app.database.schema.about_schema import (
    ...
    LeadershipInDB, LeadershipResponse
)
...
class AboutRepository:
    def __init__(self):
        ...
        self.leadership_collection_name = "leadership"
    ...
    def get_leadership_collection(self):
        """Get leadership collection"""
        db = get_database()
        collection = db[self.leadership_collection_name]
        self._ensure_indexes()
        return collection
    ...
    def _ensure_indexes(self):
        ...
        try:
            ...
            # Leadership indexes
            leadership_coll = db[self.leadership_collection_name]
            leadership_coll.create_index("isActive")
            leadership_coll.create_index("order")
    ...
    def create_leadership(self, leadership: LeadershipInDB) -> LeadershipResponse:
        """Create a new leadership member"""
        collection = self.get_leadership_collection()
        leadership_dict = leadership.dict()
        
        result = collection.insert_one(leadership_dict)
        leadership_dict["_id"] = result.inserted_id
    ...
    def get_leadership_by_id(self, leadership_id: str) -> Optional[dict]:
        """Get a leadership member by ID"""
        try:
            collection = self.get_leadership_collection()
            leadership = collection.find_one({"_id": ObjectId(leadership_id)})
            return leadership
        except Exception as e:
            logger.error(f"Error fetching leadership: {e}")
            return None

    def get_all_leadership(self, is_active: Optional[bool] = None) -> List[dict]:
    ...
    def update_leadership(self, leadership_id: str, leadership_data: dict) -> bool:
        """Update a leadership member"""
        try:
            collection = self.get_leadership_collection()
            leadership_data["updatedAt"] = datetime.utcnow()
            
            result = collection.update_one(
                {"_id": ObjectId(leadership_id)},
                {"$set": leadership_data}
            )
            
            return result.modified_count > 0
        except Exception as e:
            logger.error(f"Error updating leadership: {e}")
            return False

    def delete_leadership(self, leadership_id: str) -> bool:
        """Delete a leadership member"""
        try:
            collection = self.get_leadership_collection()
            result = collection.delete_one({"_id": ObjectId(leadership_id)})
            return result.deleted_count > 0
        except Exception as e:
            logger.error(f"Error deleting leadership: {e}")
            return False
