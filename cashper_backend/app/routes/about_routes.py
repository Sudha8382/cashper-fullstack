...
from app.database.schema.about_schema import (
    ...
    LeadershipRequest, LeadershipResponse, LeadershipInDB
)
...
def get_leadership():
    """
    Get all active leadership members (PUBLIC - No authentication required)
    
    Returns list of leadership team members
    """
    try:
        leadership = about_repository.get_all_leadership(is_active=True)
        ...
        return [
            LeadershipResponse(
                ...
                position=l["position"],
                image=l.get("image"),
                experience=l.get("experience"),
                education=l.get("education"),
                bio=l.get("bio"),
                isActive=l.get("isActive", True),
                order=l.get("order", 0),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch leadership. Error: {str(e)}"
        )
...
def create_leadership(
    ...
    try:
        ...
        created_leadership = about_repository.create_leadership(leadership_in_db)
        return created_leadership
        
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to create leadership. Error: {str(e)}"
        )


@router.put("/leadership/{leadership_id}", response_model=LeadershipResponse)
def update_leadership(
    ...
    existing_leadership = about_repository.get_leadership_by_id(leadership_id)
    
    if not existing_leadership:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Leadership not found"
        )
    
    leadership_data = leadership.dict()
    success = about_repository.update_leadership(leadership_id, leadership_data)
    
    if not success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to update leadership"
        )
    
    updated_leadership = about_repository.get_leadership_by_id(leadership_id)
    
    return LeadershipResponse(
        id=str(updated_leadership["_id"]),
        name=updated_leadership["name"],
        position=updated_leadership["position"],
        image=updated_leadership.get("image"),
        experience=updated_leadership.get("experience"),
        education=updated_leadership.get("education"),
        bio=updated_leadership.get("bio"),
        isActive=updated_leadership.get("isActive", True),
        order=updated_leadership.get("order", 0),
        createdAt=updated_leadership["createdAt"],
        updatedAt=updated_leadership.get("updatedAt")
    )


@router.delete("/leadership/{leadership_id}", status_code=status.HTTP_200_OK)
def delete_leadership(
    leadership_id: str,
    current_user: dict = Depends(get_current_user_optional)
):
    """Delete a leadership member (ADMIN ONLY)"""
    leadership = about_repository.get_leadership_by_id(leadership_id)
    ...
    success = about_repository.delete_leadership(leadership_id)
    
    if not success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to delete leadership"
        )
