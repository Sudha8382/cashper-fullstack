...
from app.database.schema.business_loan_schema import (
    StatusUpdate,
    BusinessLoanGetInTouchCreate,
    BusinessLoanGetInTouchResponse,
    BusinessLoanGetInTouchInDB,
    BusinessLoanApplicationCreate,
    BusinessLoanApplicationResponse,
    BusinessLoanApplicationInDB,
...
@router.post("/get-in-touch", response_model=BusinessLoanGetInTouchResponse)
async def submit_get_in_touch(data: BusinessLoanGetInTouchCreate):
    """Submit Get In Touch form for Business Loan"""
    try:
        # Convert to DB model
        db_data = BusinessLoanGetInTouchInDB(**data.model_dump()).model_dump()
        
        # Save to database
        result = business_loan_repository.create_get_in_touch(db_data)
        
        # Return response with proper field mapping
        return BusinessLoanGetInTouchResponse(
            id=str(result["_id"]),
...
@router.get("/get-in-touch", response_model=List[BusinessLoanGetInTouchResponse])
async def get_all_get_in_touch():
    """Get all Get In Touch inquiries"""
    try:
        inquiries = business_loan_repository.get_all_get_in_touch()
        return [
            BusinessLoanGetInTouchResponse(
                id=str(inquiry["_id"]),
...
@router.get("/get-in-touch/{inquiry_id}", response_model=BusinessLoanGetInTouchResponse)
async def get_get_in_touch_by_id(inquiry_id: str):
    """Get Get In Touch inquiry by ID"""
    try:
        inquiry = business_loan_repository.get_get_in_touch_by_id(inquiry_id)
        if not inquiry:
...
async def upload_document(file: UploadFile = File(...)):
    """Upload a document for Business Loan application"""
    try:
        # Save the file
...
async def get_uploaded_documents(application_id: Optional[str] = None):
    ...
    try:
        if application_id:
            ...
            application = business_loan_repository.get_application_by_application_id(application_id)
            if not application:
...
async def submit_business_loan_application(data: BusinessLoanApplicationCreate):
    ...
    try:
        ...
        # Save to database
        result = business_loan_repository.create_application(db_data)
        
        # Manual field mapping for response
        return BusinessLoanApplicationResponse(
            id=str(result["_id"]),
...
@router.get("/applications/{application_id}", response_model=BusinessLoanApplicationResponse)
async def get_application_by_id(application_id: str):
    """Get Business Loan application by ID"""
    try:
        app = business_loan_repository.get_application_by_id(application_id)
        if not app:
            raise HTTPException(status_code=404, detail="Application not found")
...
async def update_application_status(application_id: str, status_update: StatusUpdate):
    """Update Business Loan application status"""
    try:
        ...
        # Get database
        db = get_database()
        collection = db["business_loan_applications"]
...
async def get_business_loan_information():
    """Get Business Loan information including features, benefits, and requirements"""
    return {
        "title": "Business Loan",
        "description": "Fuel your business growth with our customized business loan solutions. Interest rates starting from 9.5% p.a.",
        "features": [
            {
            ...
            },
            {
                ...
                "description": "Loan tenure up to 7 years",
                "icon": "calendar"
            },
            {
                "title": "Fast Disbursal",
                "description": "Get funds in 48 hours",
                "icon": "clock"
            },
            {
                "title": "High Loan Amount",
                "description": "Loan up to ₹50 Lakhs",
                "icon": "rupee"
            }
        ],
        "eligibility": [
            "Business Vintage: Minimum 2 years",
            "Turnover: Minimum ₹10 Lakhs/year",
...
async def get_eligibility_criteria():
    """Get Business Loan eligibility criteria"""
    try:
        # Seed data if not exists
        business_loan_repository.seed_eligibility_criteria()
        
        criteria = business_loan_repository.get_all_eligibility_criteria()
        return [
            EligibilityCriteriaResponse(
                id=str(c["_id"]),
                label=c.get("label", ""),
...
@router.get("/application-by-number/{application_number}", response_model=BusinessLoanApplicationResponse)
async def get_application_by_number(application_number: str):
    """Get Business Loan application by application number"""
    try:
        app = business_loan_repository.get_application_by_application_id(application_number)
        if not app:
            raise HTTPException(status_code=404, detail="Application not found")
